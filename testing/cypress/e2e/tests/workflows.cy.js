...
describe("Verify Flow Within Form", () => {
  ...
  it("Verify Back Buttons", () => {
    ...
    pageObjects
      .backBtn()
      .should("contain.text", "Edit registration information");

    pageObjects.backBtn().click();
    pageObjects.backBtn().should("contain.text", "Back to identification");

    pageObjects.backBtn().click();
    pageObjects
      .backBtn()
      .should("contain.text", "Back to address and location");

    pageObjects.backBtn().click();
    pageObjects
      .backBtn()
      .should("contain.text", "Back to personal information");

    pageObjects.backBtn().click();
    pageObjects
      .backBtn()
      .should("contain.text", "Back to registration options");

    pageObjects.backBtn().click();
    pageObjects
      .backBtn()
      .should("contain.text", "Back to state eligibility requirements");
  });

  it("Verify Edit Buttons", () => {
    // personal information
    pageObjects.editBtn().then((editBtn) => {
      cy.get(editBtn[0]).click();
      cy.get('[data-testid="form"]')
        .find("h2")
        .should("contain.text", "Personal information");
      pageObjects.nextBtn().click().click().click().click();
    });
    ...
    pageObjects.editBtn().then((editBtn) => {
      ...
      cy.get('[data-testid="form"]')
        .find("h2")
        .should("contain.text", "Address and location");
      pageObjects.nextBtn().click().click().click();
    });
    // identification
    pageObjects.editBtn().then((editBtn) => {
      ...
      cy.get('[data-testid="form"]')
        .find("h2")
        .should("contain.text", "Identification");
      ...
      .then((ul) => {
        ...
        cy.get(ul[0])
          ...
          .then((li) => {
            ...
            cy.get(li[2]).should(
              "contain.text",
              data.personalInformationMiddle,
            );
            cy.get(li[3]).should("contain.text", data.personalInformationLast);
            cy.get(li[4]).should(
              "contain.text",
              data.personalInformationSuffix,
            );
          });
        // previous name
        cy.get(ul[1])
        ...
        cy.get(ul[2])
          ...
          .then((li) => {
            ...
            cy.get(li[1]).should(
              "contain.text",
              data.personalInformationNumber2,
            );
