export const focusError = (formId) => {
  if (!document.getElementById(formId).checkValidity()) {
    const firstInvalidChild = document
      .getElementById(formId)
      .querySelectorAll(":invalid")[0];
    let errorContainer;
    errorContainer =
      firstInvalidChild.id === "eligibility-checkbox" ||
      firstInvalidChild.id === "acknowledge-checkbox"
        ? firstInvalidChild.parentNode.parentNode
        : firstInvalidChild.parentNode;
...
export const toggleError = (e, error) => {
  ...
  let dateOfBirthInputs =
    input.id === "date_of_birth_month" ||
    input.id === "date_of_birth_day" ||
    input.id === "date_of_birth_year";
  let errorContainer;
  errorContainer =
    input.id === "eligibility-checkbox" ||
    input.id === "date_of_birth" ||
    input.id === "acknowledge-checkbox"
      ? input.parentNode.parentNode
      : input.parentNode;
  errorContainer = dateOfBirthInputs
