...
function FieldsetContainer({ fieldType, inputData, saveFieldData, dateFormat, fieldData }) {
    ...
    return (
        <>
            <div className={fieldError.length > 0 ? "vote-error-container" : "input-parent"}>
                <Fieldset legend={inputData.required ? [<span key={0} className="text-bold">{inputData.label}</span>, <span key={1} className='required-text'>*</span>] : (inputData.stringContent)} style={{ marginTop: '30px' }}>
