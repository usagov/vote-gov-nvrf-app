...
function ProgressBar(props) {
  ...
  let currentStepMessage = steps[stepList[currentStep]].aria_label.replace(
    "@label",
    steps[stepList[currentStep]].label,
  );
        ...
        {Object.keys(stepList)
          ...
          .map((step) => (
            <StepIndicatorStep
              ...
              tabIndex={
                stepProgress(step) === "complete" && currentStep !== finalStep
                  ? 0
                  : null
              }
              onKeyDown={(e) => {
              ...
              onClick={
                stepProgress(step) === "complete" && currentStep !== finalStep
                  ? handleGoBackSteps(currentStep - step)
                  : null
              }
            />
          ))}
