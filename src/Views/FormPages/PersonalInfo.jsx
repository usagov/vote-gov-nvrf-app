...
function PersonalInfo(props) {
  ...
  const changeRegistrationVisible =
    props.registrationPath === "update" ? true : false;
  const nvrfStateFields = props.stateData.nvrf_fields;

  //Drupal field data
  const nameChangeLabel = getFieldLabel(
    fields,
    "e87ca867-c5a5-4e42-98d5-d742edd03de3",
  );
  const nameSectionField = fields.find(
    (item) => item.uuid === "8dda085c-edf3-4678-b30a-0a457699be46",
  );
  const prevNameSectionField = fields.find(
    (item) => item.uuid === "af4e6259-5b07-4955-9d28-254504ec9df8",
  );
  const firstNameField = fields.find(
    (item) => item.uuid === "b7bdae35-e4be-4827-ae11-75d9c3e33bf0",
  );
  const phoneNumberField = fields.find(
    (item) => item.uuid === "2d61b54a-e568-410f-825a-0ca82dfd3f63",
  );
  const raceField = fields.find(
    (item) => item.uuid === "2bfff6c6-6782-4b14-ac45-642efd278f6a",
  );
  ...
  const nameFieldState = nvrfStateFields.find(
    (item) => item.uuid === firstNameField.uuid,
  );
  const telephoneFieldState = nvrfStateFields.find(
    (item) => item.uuid === phoneNumberField.uuid,
  );
  const raceFieldState = nvrfStateFields.find(
    (item) => item.uuid === raceField.uuid,
  );
