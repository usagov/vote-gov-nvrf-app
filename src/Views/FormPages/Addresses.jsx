...
function Addresses(props) {
  ...
  const changeRegistrationVisible =
    props.registrationPath === "update" || props.stateData.name == "Maine"
      ? true
      : false;
  const nvrfStateFields = props.stateData.nvrf_fields;

  //Drupal field data
  const homeAddressSectionField = fields.find(
    (item) => item.uuid === "63552bb6-6afb-46e1-8148-860242917a22",
  );
  const streetAddressField = fields.find(
    (item) => item.uuid === "6dcb9e8c-b40a-4cda-ba5c-06b98c3375f4",
  );

  const prevAddressSectionField = fields.find(
    (item) => item.uuid === "023fda0f-e8bd-4654-ab5c-46f44a0b7bd6",
  );
  const prevAddressField = fields.find(
    (item) => item.uuid === "c3011c62-d174-420c-817a-bffbcd45687a",
  );

  const mailAddressSectionField = fields.find(
    (item) => item.uuid === "1a856408-6fb2-4b09-b05a-8d8ee9eb9bb5",
  );
  const noAddressField = fields.find(
    (item) => item.uuid === "35c2b98d-477c-45f3-9f93-f720406080f1",
  );
  const differentMailAddressField = fields.find(
    (item) => item.uuid === "e7340274-ee3f-4d73-a967-c9d7c249be7b",
  );

  const noAddressSection = fields.find(
    (item) => item.uuid === "3724c7cd-5ec7-4e3e-85cd-db0cab63e99b",
  );
  const movedAndNoAddressSection = fields.find(
    (item) => item.uuid === "6dd20906-654e-427e-bb82-1e62aee9ed72",
  );

  //Field requirements by state data
  const addressFieldsState = nvrfStateFields.find(
    (item) => item.uuid === streetAddressField.uuid,
  );

  // Instructions for optional checkboxes
  const noAddressCheckboxInstructions = sanitizeDOM(noAddressSection.label);
  const movedAndNoAddressCheckboxInstructions = sanitizeDOM(
    movedAndNoAddressSection.label,
  );
        ...
        <>
          <div className="input-parent">
            <Fieldset className="fieldset">
              <legend className="usa-hint" id="addresses-checkbox-hint">
                {!changeRegistrationVisible
                  ? noAddressCheckboxInstructions
                  : movedAndNoAddressCheckboxInstructions}
              </legend>
              ...
              <Checkbox
                id="no-address"
                aria-describedby="no-address_alert"
                className="margin-bottom-4"
                name="no-addr"
                data-test="checkbox-no-address"
                checked={props.hasNoAddress}
          ...
          {!props.hasNoAddress && (
            <>
                  ...
                  <div className="usa-alert__body">
                    <div
                      ...
                      dangerouslySetInnerHTML={{
                        __html: homeAddressSectionField.section_alert,
                      }}
                    />
                  </div>
                </div>
              ...
              {homeAddressSectionField.instructions && (
                <div
                  dangerouslySetInnerHTML={{
                    __html: homeAddressSectionField.instructions,
                  }}
                />
              )}
              ...
              <div className="input-parent">
                <Fieldset className="fieldset">
                  <Checkbox
                    data-test="checkbox-mail-address"
          ...
          {(props.hasMailAddress || props.hasNoAddress) && (
            <>
                ...
                <div className="usa-alert usa-alert--info" role="alert">
                  <div className="usa-alert__body">
                    <div
                      className="usa-alert__text"
                      dangerouslySetInnerHTML={{
                        __html: mailAddressSectionField.section_alert,
                      }}
                    />
                  </div>
                </div>
              ...
              {mailAddressSectionField.section_description && (
                <div
                  dangerouslySetInnerHTML={{
                    __html: mailAddressSectionField.section_description,
                  }}
                />
              )}
          ...
          {props.hasPreviousAddress && (
            <>
                ...
                <div className="usa-alert usa-alert--info" role="alert">
                  <div className="usa-alert__body">
                    <div
                      className="usa-alert__text"
                      dangerouslySetInnerHTML={{
                        __html: prevAddressSectionField.section_alert,
                      }}
                    />
                  </div>
                </div>
              ...
              {prevAddressSectionField.instructions && (
                <div
                  dangerouslySetInnerHTML={{
                    __html: prevAddressSectionField.instructions,
                  }}
                />
              )}
