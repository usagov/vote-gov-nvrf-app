...
function Identification(props) {
  ...
  const idTypeField = fields.find(
    (item) => item.uuid === "27d3a15c-f8c0-4035-9b0a-c2c0f674519c",
  );
  const driverLicenseField = fields.find(
    (item) => item.uuid === "acd7f272-7a37-43f0-b51a-c78daf31e5fd",
  );
  const stateIDField = fields.find(
    (item) => item.uuid === "e2da00fa-0f1b-4e98-9472-c00649266eb4",
  );
  const ssnField = fields.find(
    (item) => item.uuid === "1e030197-52e7-426e-923c-b67ef521ae3b",
  );
  const ssnFullField = fields.find(
    (item) => item.uuid === "fe8cf91e-f872-4ed7-848c-09c99a7d83c8",
  );
  const noIdField = fields.find(
    (item) => item.uuid === "eb0ce8c5-b4f7-4aae-a0b9-84f0434d2edb",
  );
  ...
  const driverIDFieldReq = nvrfStateFields.find(
    (item) => item.uuid === driverLicenseField.uuid,
  );
  const stateIDFieldDReq = nvrfStateFields.find(
    (item) => item.uuid === stateIDField.uuid,
  );
  const ssnFullFieldReq = nvrfStateFields.find(
    (item) => item.uuid === ssnFullField.uuid,
  );
  const ssnFieldReq = nvrfStateFields.find(
    (item) => item.uuid === ssnField.uuid,
  );
  const noIdFieldReq = nvrfStateFields.find(
    (item) => item.uuid === noIdField.uuid,
  );
    ...
    <>
            ...
            >
              <React.Fragment key=".0">
                <option key="default" value="">
                  {idTypeField.options[0].key}
                </option>
                {driverIDFieldReq && (
                  <option key="driver-id-num" value="driver-id-num">
                    {driverLicenseField.label}
                  </option>
                )}
                {stateIDFieldDReq && (
                  <option key="state-id-num" value="state-id-num">
                    {stateIDField.label}
                  </option>
                )}
                {ssnFullFieldReq && (
                  <option key="ssn-full" value="ssn-full">
                    {ssnFullField.label}
                  </option>
                )}
                {ssnFieldReq && (
      ...
      <div id="state-id" className="input-parent">
        {(props.idType === "driver-id-num" ||
          ...
          (stateData.abbrev === "mo" && props.idType != "none")) && (
          <>
            {(props.idType === "driver-id-num" ||
              stateData.abbrev === "mo") && (
              <>
