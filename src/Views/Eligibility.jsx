...
function Eligibility(props) {
  const { pageContent, stateContent, fieldContent, stepContent } =
    useContext(DataContext);
  const returnPath = document
    .getElementById("root")
    .getAttribute("data-returnPath");

  const step = stepContent.eligibility;

  const content = pageContent.data.find(
    (item) => item.uuid === "94eab1c9-8343-4747-94b4-08732a175614",
  );
  const contentBody = sanitizeDOM(content.body);
  const contentBodyParts = contentBody.split("@reg_confirm_eligibility");

  const eligibility = fieldContent.data.find(
    (item) => item.uuid === "39fc63ad-ed5a-4ad5-98d3-aa236c96c61c",
  );
    ...
    <ul>
      <li
        dangerouslySetInnerHTML={{
          __html:
            stateContent.data.postmarked_mail_deadline ||
            stateContent.data.received_mail_deadline,
        }}
      />
    </ul>
  );
    ...
    <>
      ...
      <div className={"margin-top-5 maxw-tablet margin-x-auto"}>
        ...
        <div
          ...
          dangerouslySetInnerHTML={{
            __html: contentBodyParts[0]
              .replace("@state_name", stateContent.data.name)
              .replace(
                "@reg_eligibility_desc",
                stateContent.data.reg_eligibility_desc,
              ),
          }}
        />
        ...
        >
          <div className="input-parent">
            ...
            >
              ...
              <Checkbox
                ...
                label={getFieldLabel(
                  fieldContent.data,
                  "39fc63ad-ed5a-4ad5-98d3-aa236c96c61c",
                )}
                aria-required="true"
          ...
          <div
            className={"margin-top-5"}
            dangerouslySetInnerHTML={{
              __html: contentBodyParts[1]
                .replace("@state_name", stateContent.data.name)
                .replace(
                  "@mail_deadline",
                  renderToStaticMarkup(mailDeadline()),
                ),
            }}
          />
